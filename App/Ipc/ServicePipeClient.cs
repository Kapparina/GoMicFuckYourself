...
public interface IServicePipeClient : IDisposable
{
    ...
    Task<PipeResponse<ServiceConfig>> SaveConfigAsync(ServiceConfig config, CancellationToken cancellationToken = default);
...
public sealed class ServicePipeClient : IServicePipeClient
{
    ...
    public async Task<PipeResponse<MicEnforcementStatus>> GetStatusAsync(CancellationToken cancellationToken = default) =>
        await SendAsync<MicEnforcementStatus>("GetStatus", null, cancellationToken);

    public async Task<PipeResponse<ServiceConfig>> GetConfigAsync(CancellationToken cancellationToken = default) =>
        await SendAsync<ServiceConfig>("GetConfig", null, cancellationToken);

    public async Task<PipeResponse<List<CaptureDeviceInfo>>> ListCaptureDevicesAsync(CancellationToken cancellationToken = default) =>
        await SendAsync<List<CaptureDeviceInfo>>("ListCaptureDevices", null, cancellationToken);

    public async Task<PipeResponse<ServiceConfig>> SaveConfigAsync(ServiceConfig config, CancellationToken cancellationToken = default) =>
        await SendAsync<ServiceConfig>("SaveConfig", config, cancellationToken);

    public async Task<PipeResponse<MicEnforcementStatus>> ForceEnforceAsync(CancellationToken cancellationToken = default) =>
        await SendAsync<MicEnforcementStatus>("ForceEnforce", null, cancellationToken);
    ...
    private static async Task<PipeResponse<T>> SendAsync<T>(string type, object? payload, CancellationToken cancellationToken)
    {
        ...
        var envelope = JsonSerializer.Deserialize<PipeResponseEnvelope>(responseJson, JsonOptions)
            ?? throw new InvalidOperationException("The service response was invalid.");
