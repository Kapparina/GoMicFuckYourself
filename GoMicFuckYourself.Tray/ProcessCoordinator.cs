...
internal static class ProcessCoordinator
{
    ...
    public static void TerminateOtherTrayInstances() => TerminateProcesses(TrayProcessName, Process.GetCurrentProcess().Id);

    public static void TerminateAgentInstances() => TerminateProcesses(AgentProcessName, currentProcessIdToSkip: null);
    ...
    private static void TerminateProcesses(string processName, int? currentProcessIdToSkip)
    {
        foreach (var process in Process.GetProcessesByName(processName))
        {
            try
            {
                if (currentProcessIdToSkip.HasValue && process.Id == currentProcessIdToSkip.Value)
                {
                    continue;
                }
